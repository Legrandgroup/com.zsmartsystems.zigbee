...
public class AshFrameHandler {
	...
	/**
	 * Maximum number of consecutive timeouts allowed while waiting to receive an
	 * ACK
	 */
	...
	/**
	 * The queue of {@link AshFrameData} frames that we have sent. These are kept in
	 * case a resend is required.
	 */
	...
	/**
	 * Flag reflecting that parser has been closed and parser parserThread should
	 * exit.
	 */
	...
	private int[] getPacket() throws IOException {
		...
		while (!closeHandler) {
			...
			case ASH_CANCEL_BYTE:
				// Cancel Byte: Terminates a frame in progress. A Cancel Byte causes all data
				// received since the
				// previous Flag Byte to be ignored. Note that as a special case, RST and RSTACK
				// frames are preceded
				...
				break;
			case ASH_FLAG_BYTE:
				// Flag Byte: Marks the end of a frame.When a Flag Byte is received, the data
				// received since the
			...
			case ASH_SUBSTITUTE_BYTE:
				// Substitute Byte: Replaces a byte received with a low-level communication
				// error (e.g., framing
				// error) from the UART.When a Substitute Byte is processed, the data between
				// the previous and the
				// next Flag Bytes is ignored.
				inputError = true;
				break;
			case ASH_XON_BYTE:
				// XON: Resume transmissionUsed in XON/XOFF flow control. Always ignored if
				// received by the NCP.
				break;
			case ASH_OFF_BYTE:
				// XOFF: Stop transmissionUsed in XON/XOFF flow control. Always ignored if
				// received by the NCP.
				break;
			case ASH_TIMEOUT:
				break;
	...
	// Otherwise this method could be called twice from different threads that could
	// end up with
	...
	/**
	 * Add an EZSP frame to the send queue. The sendQueue is a FIFO queue. This
	 * method queues a {@link EzspFrameRequest} frame without waiting for a response
	 * and no transaction management is performed.
	 *
	 * @param request
	...
	/**
	 * Acknowledge frames we've sent and removes the from the sent queue. This
	 * method is called for each DATA or ACK frame where we have the 'ack' property.
	 *
	 * @param ackNum
	...
	/**
	 * Sends an EZSP request to the NCP and waits for the response. The response is
	 * correlated with the request and the returned {@link EzspFrame} contains the
