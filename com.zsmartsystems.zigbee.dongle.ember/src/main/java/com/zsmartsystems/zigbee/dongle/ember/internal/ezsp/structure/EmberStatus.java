...
public enum EmberStatus {
	...
	/**
	 * The manufacturing and stack token format in nonvolatile memory is different
	 * than what the stack expects (returned at initialization).
	 */
	EMBER_EEPROM_MFG_STACK_VERSION_MISMATCH(0x0004),

	/**
	 * The static memory definitions in ember-staticmemory.h are incompatible with
	 * this stack version
	 */
	EMBER_INCOMPATIBLE_STATIC_MEMORY_DEFINITIONS(0x0005),

	/**
	 * The manufacturing token format in non-volatile memory is different than what
	...
	/**
	 * There was not enough space to store a received character and the character
	 * was dropped.
	 */
	...
	/**
	 * We expected to receive an ACK following the transmission, but the MAC level
	...
	/**
	 * The Simulated EEPROM is telling the application that there is at least one
	 * flash page to be erased. The GREEN status means the current page has not
	 * filled above the ERASE_CRITICAL_THRESHOLD. The application should call the
	 * function halSimEepromErasePage when it can to erase a page.
	 */
	EMBER_SIM_EEPROM_ERASE_PAGE_GREEN(0x0043),

	/**
	 * The Simulated EEPROM is telling the application that there is at least one
	 * flash page to be erased. The RED status means the current page has filled
	 * above the ERASE_CRITICAL_THRESHOLD. Due to the shrinking availability of
	 * write space, there is a danger of data loss. The application must call the
	 * function halSimEepromErasePage as soon as possible to erase a page.
	 */
	EMBER_SIM_EEPROM_ERASE_PAGE_RED(0x0044),

	/**
	 * The Simulated EEPROM has run out of room to write any new data and the data
	 * trying to be set has been lost. This error code is the result of ignoring the
	 * SIM_EEPROM_ERASE_PAGE_RED error code. The application must call the function
	 ...
	 * target memory attempting to be programmed is already programmed. The flash
	 * write routines were asked to flip a bit from a 0 to 1, which is physically
	 * impossible and the write was therefore inhibited. The data in the flash
	 * cannot be trusted after this error.
	 */
	EMBER_ERR_FLASH_WRITE_INHIBITED(0x0046),

	/**
	 * A fatal error has occurred while trying to write data to the Flash and the
	 * write verification has failed. The data in the flash cannot be trusted after
	 * this error, and it is possible this error is the result of exceeding the life
	 * cycles of the flash.
	 */
	EMBER_ERR_FLASH_VERIFY_FAILED(0x0047),

	/**
	 * Attempt 1 to initialize the Simulated EEPROM has failed. This failure means
	 * the information already stored in Flash (or a lack thereof), is fatally
	 * incompatible with the token information compiled into the code image being
	 * run.
	 */
	EMBER_SIM_EEPROM_INIT_1_FAILED(0x0048),

	/**
	 * Attempt 2 to initialize the Simulated EEPROM has failed. This failure means
	 * Attempt 1 failed, and the token system failed to properly reload default
	 * tokens and reset the Simulated EEPROM.
	 */
	EMBER_SIM_EEPROM_INIT_2_FAILED(0x0049),

	/**
	 * Attempt 3 to initialize the Simulated EEPROM has failed. This failure means
	 * one or both of the tokens TOKEN_MFG_NVDATA_VERSION or
	...
	/**
	 * A fatal error has occurred while trying to write data to the flash, possibly
	 * due to write protection or an invalid address. The data in the flash cannot
	 * be trusted after this error, and it is possible this error is the result of
	 * exceeding the life cycles of the flash.
	 */
	EMBER_ERR_FLASH_PROG_FAIL(0x004B),

	/**
	 * A fatal error has occurred while trying to erase flash, possibly due to write
	 * protection. The data in the flash cannot be trusted after this error, and it
	 * is possible this error is the result of exceeding the life cycles of the
	 * flash
	 */
	...
	/**
	 * The bootloader cannot complete the bootload operation because either an image
	 * was not found or the image exceeded memory bounds.
	 */
	...
	/**
	 * The message to be transmitted is too big to fit into a single over-the-air
	 * packet
	 */
	...
	/**
	 * The packet cannot be transmitted because the physical MAC layer is currently
	 * transmitting a packet. (This is used for the MAC backoff algorithm.)
	 */
	EMBER_PHY_TX_BUSY(0x008C),

	/**
	 * The transmit attempt failed because all CCA attempts indicated that the
	 * channel was busy
	 */
	EMBER_PHY_TX_CCA_FAIL(0x008D),

	/**
	 * The software installed on the hardware doesn't recognize the hardware radio
	 * type.
	 */
	 ...
	 * incompatibility. ZigBee devices joining ZigBee Pro networks (or vice versa)
	 * must join as End Devices, not Routers.
	 */
	EMBER_CANNOT_JOIN_AS_ROUTER(0x0098),

	/**
	 * The local node ID has changed. The application can obtain the new node ID by
	 * calling emberGetNodeId().
	 */
	EMBER_NODE_ID_CHANGED(0x0099),

	/**
	 * The local PAN ID has changed. The application can obtain the new PAN ID by
	 * calling emberGetPanId().
	 */
	...
	/**
	 * An attempt was made to join a Secured Network using a pre-configured key, but
	 * the Trust Center sent back a Network Key in-the-clear when an encrypted
	 * Network Key was required.
	 */
	EMBER_RECEIVED_KEY_IN_THE_CLEAR(0x00AC),

	/**
	 * An attempt was made to join a Secured Network, but the device did not receive
	 * a Network Key.
	 */
	...
	/**
	 * The application tried to use a binding that has been remotely modified and
	 * the change has not yet been reported to the application.
	 */
	...
	/**
	 * There was an error in trying to encrypt at the APS Level. This could result
	 * from either an inability to determine the long address of the recipient from
	 * the short address (no entry in the binding table) or there is no link key
	 * entry in the table associated with the destination, or there was a failure to
	 * load the correct key into the encryption core.
	 */
	EMBER_APS_ENCRYPTION_ERROR(0x00A6),

	/**
	 * There was an attempt to form a network using commercial security without
	 * setting the Trust Center master key first.
	 */
	...
	/**
	 * There was an attempt to set an entry in the key table using an invalid long
	 * address. An entry cannot be set using either the local device's or Trust
	 * Center's IEEE address. Or an entry already exists in the table with the same
	 * IEEE address. An Address of all zeros or all F's are not valid addresses in
	 * 802.15.4.
	 */
	EMBER_KEY_TABLE_INVALID_ADDRESS(0x00B3),

	/**
	 * There was an attempt to set a security configuration that is not valid given
	 * the other security settings.
	 */
	EMBER_SECURITY_CONFIGURATION_INVALID(0x00B7),

	/**
	 * There was an attempt to broadcast a key switch too quickly after broadcasting
	 * the next network key. The Trust Center must wait at least a period equal to
	 * the broadcast timeout so that all routers have a chance to receive the
	 * broadcast of the new network key.
	 */
	 ...
	 * messages it must be authorized using a key agreement protocol (such as CBKE).
	 */
	 ...
	 * sent to this node along a many-to-one route failed en route. The route error
	 * frame was delivered by an ad-hoc search for a functioning route.
	 */
	EMBER_MANY_TO_ONE_ROUTE_FAILURE(0x00AA),

	/**
	 * A critical and fatal error indicating that the version of the stack trying to
	 * run does not match with the chip it is running on. The software (stack) on
	 * the chip must be replaced with software that is compatible with the chip
	 */
	...
	/**
	 * The stack accepted the command and is currently processing the request. The
	 * results will be returned via an appropriate handler.
	 */
