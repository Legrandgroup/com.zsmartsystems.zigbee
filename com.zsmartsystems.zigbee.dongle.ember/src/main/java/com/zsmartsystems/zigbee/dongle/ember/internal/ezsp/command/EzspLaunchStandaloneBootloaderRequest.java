...
public class EzspLaunchStandaloneBootloaderRequest extends EzspFrameRequest {
	...
	/**
	 * Controls the mode in which the standalone bootloader will run. See the app.
	 * note for full details. Options are: STANDALONE_BOOTLOADER_NORMAL_MODE: Will
	 * listen for an over-the-air image transfer on the current channel with current
	 * power settings. STANDALONE_BOOTLOADER_RECOVERY_MODE: Will listen for an
	 * over-the-air im-age transfer on the default channel with default power
	 * settings. Both modes also allow an image transfer to begin with XMODEM over
	 * the serial protocol's Bootloader Frame.
	 * <p>
