...
public class EmberNetworkInitialisation {
	...
	/**
	 * This utility function uses emberStartScan, emberStopScan,
	 * emberScanCompleteHandler, emberEnergyScanResultHandler, and
	 * emberNetworkFoundHandler to discover other networks or determine the
	 * background noise level. It then uses emberFormNetwork to create a new network
	 * with a unique PAN-ID on a channel with low background noise.
	 * <p>
	 * Setting the PAN-ID or Extended PAN-ID to 0 will set these values to a random
	 * value.
	 * <p>
	 * If channel is set to 0, the quietest channel will be used.
	 *
	 * @param networkParameters
	 *            the required {@link EmberNetworkParameters}
	 * @param networkKey
	 *            the {@link EmberKeyData} with the network key. This can not be set
	 *            to all 00 or all FF.
	 */
	public void formNetwork(EmberNetworkParameters networkParameters, EmberKeyData networkKey) {
		...
		// Check if any current networks were found and avoid those channels, PAN ID and
		// especially Extended PAN ID
	 ...
	 *
	 * @param scanDuration
	 *            duration of the scan on each channel
	 ...
	 *
	 * @param networkKey
	 *            the initial {@link EmberKeyData}
	 * @return true if the security state was set successfully
	 */
	private boolean setSecurityState(EmberKeyData networkKey) {
		// Define the ZigBeeAliance09 key for HA link-key
		// This is used to encrypt the network key when it is transferred to a newly
		// joining device.
		...
		linkKey.setContents(new int[] { 0x5A, 0x69, 0x67, 0x42, 0x65, 0x65, 0x41, 0x6C, 0x6C, 0x69, 0x61, 0x6E, 0x63,
				0x65, 0x30, 0x39 });
		// linkKey.setContents(new int[] { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
	 ...
	 *            the panId as int
	 * @param extendedPanId
	 *            the extended pan ID as {@link ExtendedPanId}
	 * @param channel
