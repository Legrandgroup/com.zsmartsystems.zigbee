 ...
 * <p>
 * Identifies which configuration value to read or change with
 * getConfigurationValue and setConfigurationValue.
 * <p>
...
public enum EzspConfigId {
	 ...
	 * packet buffers, such that the resulting count will be the largest whole
	 * number of packet buffers that can fit into the available memory.
	 */
	 ...
	 * cache size limits the number of overlapping joins the Trust Center can
	 * process within a narrow time window (e.g. two seconds), and thus should be
	 * set to the maximum number of near simultaneous joins the Trust Center is
	 * expected to accommodate. (Note, the total number of such address associations
	 * maintained by the NCP is the sum of the value of this setting and the value
	 * of ::EZSP_CONFIG_ADDRESS_TABLE_SIZE.)
	 */
	 ...
	 * device is a Trust Center) or Application Link Keys (if the device is a normal
	 * node).
	 */
	...
	/**
	 * The time the coordinator will wait (in seconds) for a second end device bind
	 * request to arrive.
	 */
	 ...
	 * field upgrade devices with new Smart Energy certificates. This value cannot
	 * be set, it can only be queried.
	 */
	 ...
	 * EmberZdoConfigurationFlags enumeration. To see if the application is required
	 * to send a ZDO response in reply to an incoming message, the application must
	 * check the APS options bitfield within the incomingMessageHandler callback to
	 * see if the EMBER_APS_OPTION_ZDO_RESPONSE_REQUIRED flag is set.
	 */
	 ...
	 * sleepy broadcast address (0xFFFF). The RxOnWhenIdle=true address is the
	 ...
	 * link key that a device can use to join its network. A transient key is added
	 * with a call to emberAddTransientLinkKey. After the transient key is added, it
	 * will be removed once this amount of time has passed. A joining device will
	 * not be able to use that key to join until it is added again on the trust
	 * center. The default value is 300 seconds, i.e., 5 minutes.
	 */
	 ...
	 * (insecure) rejoin for a device that is using the well-known link key. This
	 * timeout takes effect once rejoins using the well-known key has been allowed.
	 * This command updates the emAllowTcRejoinsUsingWellKnownKeyTimeoutSec value.
	 */
