...
public enum EzspValueId {
	 ...
	 * will store the rejoin reason used by the call to emberFindAndRejoinNetwork()
	 */
	EZSP_VALUE_NEXT_HOST_REJOIN_REASON(0x0012),

	/**
	 * This is the reason that the last rejoin took place. This value may only be
	 * retrieved, not set. The rejoin may have been initiated by the stack (NCP) or
	 * the application (host). If a host initiated a rejoin the reason will be set
	 * by default to EMBER_REJOIN_DUE_TO_APP_EVENT_1. If the application wishes to
	 * denote its own rejoin reasons it can do so by calling
