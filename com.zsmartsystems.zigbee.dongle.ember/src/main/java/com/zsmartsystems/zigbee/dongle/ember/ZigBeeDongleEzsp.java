...
public class ZigBeeDongleEzsp implements ZigBeeTransportTransmit, ZigBeeTransportFirmwareUpdate, EzspFrameHandler {
    ...
    public ZigBeeDongleEzsp(final ZigBeePort serialPort) {
        ...
        this.zigbeeTransportReceive = null;
    ...
    public boolean startup(boolean reinitialize) {
        ...
        if (networkStateResponse.getStatus() == EmberNetworkStatus.EMBER_JOINED_NETWORK) {
        	if( null != zigbeeTransportReceive ) { zigbeeTransportReceive.setNetworkState(ZigBeeTransportState.ONLINE); }
        }
    ...
    public void handlePacket(final EzspFrame response) {
        ...
        if (response instanceof EzspIncomingMessageHandler) {
            ...
            if( null != zigbeeTransportReceive ) { zigbeeTransportReceive.receiveCommand(apsFrame); }
        ...
        if (response instanceof EzspStackStatusHandler) {
            switch (((EzspStackStatusHandler) response).getStatus()) {
                ...
                case EMBER_NETWORK_DOWN:
                	if( null != zigbeeTransportReceive ) { zigbeeTransportReceive.setNetworkState(ZigBeeTransportState.OFFLINE); }
                    break;
        ...
        if (response instanceof EzspTrustCenterJoinHandler) {
            ...
            if( null != zigbeeTransportReceive ) { zigbeeTransportReceive.nodeStatusUpdate(status, joinHandler.getNewNodeId(), joinHandler.getNewNodeEui64()); }

        }

        if (response instanceof EzspChildJoinHandler) {
            EzspChildJoinHandler joinHandler = (EzspChildJoinHandler) response;
            if( null != zigbeeTransportReceive ) { zigbeeTransportReceive.nodeStatusUpdate(ZigBeeNodeStatus.UNSECURED_JOIN, joinHandler.getChildId(),
    ...
    public void handleLinkStateChange(boolean linkState) {
    	...
    	if( null != zigbeeTransportReceive ) { zigbeeTransportReceive.setNetworkState(linkState ? ZigBeeTransportState.ONLINE : ZigBeeTransportState.OFFLINE); }
    }
