...
public class ZigBeeDongleEzsp implements ZigBeeTransportTransmit, ZigBeeTransportFirmwareUpdate, EzspFrameHandler {
	...
	/**
	 * The Ember version used in this system. Set during initialisation and saved in
	...
	public ZigBeeDongleEzsp(final ZigBeePort serialPort) {
		...
		// stackPolicies.put(EzspPolicyId.EZSP_POLL_HANDLER_POLICY,
		// EzspDecisionId.EZSP_POLL_HANDLER_CALLBACK);
	...
	/**
	 * Gets the current network parameters, or an empty parameters class if there's
	 * an error
	 *
	...
	public void sendCommand(final ZigBeeApsFrame apsFrame) {
		...
		if (apsFrame.getAddressMode() == ZigBeeNwkAddressMode.DEVICE && apsFrame.getDestinationAddress() < 0xfff8) {
			...
			// ZigBeeGroupAddress groupAddress = (ZigBeeGroupAddress)
			// zclCommand.getDestinationAddress();
		...
		// emberUnicast = (EzspSendUnicast)
		// ashHandler.sendEzspRequestAsync(emberUnicast);
	}
	...
	public void handlePacket(final EzspFrame response) {
		...
		if (response instanceof EzspTrustCenterJoinHandler) {
			...
			if (null != zigbeeTransportReceive) {
				zigbeeTransportReceive.nodeStatusUpdate(status, joinHandler.getNewNodeId(),
	...
	public boolean updateFirmware(final InputStream firmware, final ZigBeeTransportFirmwareCallback callback) {
		...
		// Send the bootload command, but ignore the response since there doesn't seem
		// to be one
	...
	// Callback from the bootload handler when the transfer is
