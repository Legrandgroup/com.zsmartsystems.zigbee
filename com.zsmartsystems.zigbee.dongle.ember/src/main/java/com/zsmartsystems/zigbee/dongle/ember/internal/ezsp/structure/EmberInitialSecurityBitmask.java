...
public enum EmberInitialSecurityBitmask {
	...
	/**
	 * This enables a Global Link Key for the Trust Center. All nodes will share the
	 * same Trust Center Link Key
	 */
	 ...
	 * Secret Key known only to the Trust Center. It is hashed with the IEEE Address
	 * of the destination device in order to create the actual Link Key used in
	 * encryption. This is bit is only used by the Trust Center. The joining device
	 * need not set this.
	 */
	EMBER_TRUST_CENTER_USES_HASHED_LINK_KEY(0x0084),

	/**
	 * This denotes that the preconfiguredKey element has valid data that should be
	 * used to configure the initial security state.
	 */
	EMBER_HAVE_PRECONFIGURED_KEY(0x0100),

	/**
	 * This denotes that the networkKey element has valid data that should be used
	 * to configure the initial security state.
	 */
	EMBER_HAVE_NETWORK_KEY(0x0200),

	/**
	 * This denotes to a joining node that it should attempt to acquire a Trust
	 * Center Link Key during joining. This is only necessary if the device does not
	 * have a pre-configured key.
	 */
	EMBER_GET_LINK_KEY_WHEN_JOINING(0x0400),

	/**
	 * This denotes that a joining device should only accept an encrypted network
	 * key from the Trust Center (using its preconfigured key). A key sent
	 * in-the-clear by the Trust Center will be rejected and the join will fail.
	 ...
	 * (both NWK and APS) when ::emberSetInitialSecurityState() is called. Normally
	 * it is advised to reset the frame counter before joining a new network.
	 * However in cases where a device is joining to the same network a again (but
	 * not using ::emberRejoinNetwork()) it should keep the NWK and APS frame
	 * counters stored in its tokens.
	 */
	 ...
	 * containing the trust center EUI64. The device will only join a network and
	 * accept commands from a trust center with that EUI64. Normally this bit is NOT
	 * set, and the EUI64 of the trust center is learned during the join process.
	 * When commissioning a device to join onto an existing network, which is using
	 * a trust center, and without sending any messages, this bit must be set and
