...
public class EmberFirmwareUpdateHandler {
	 ...
	 *
	 * @param dongle
	 *            the {@link ZigBeeDongleEzsp} to receive the completion
	 *            notification
	 * @param firmware
	 *            {@link InputStream} containing the data
	 * @param serialPort
	 *            {@link ZigBeePort} to communicate on
	 * @param callback
	 *            {@link ZigBeeTransportFirmwareCallback} to provide status updates
	 */
	...
	/**
	 * Starts the bootload. The routine will open the serial port at 115200baud. It
	 * will send a CR up to 3 times waiting for the bootloader prompt. It will then
	 * select the upload option, transfer the file, then run. The serial port will
	 * then be closed.
	 *
	 */
	public void startBootload() {
		Thread firmwareThread = new Thread("TelegesisFirmwareUpdateHandler") {
			...
			public void run() {
				...
				// Short delay here to allow all bootloader menu data to be received so there's
				// no confusion with acks.
				try {
					sleep(1500);
				} catch (InterruptedException e) {
					// Eat me!
				...
				// Short delay here to allow completion. This is mainly required if there was an
				// abort.
				try {
					sleep(5000);
				} catch (InterruptedException e) {
				...
				// Transfer was completed, or aborted. Either way all we can do is run the
				// firmware and it should return
				// to the main prompt
	 ...
	 *
	 * @param status
	 *            the final {@link ZigBeeTransportFirmwareStatus} at completion
	 */
