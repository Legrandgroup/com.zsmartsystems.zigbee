...
public class AshFrame {
	 ...
	 * <ol>
	 * <li>The Control Byte is added before the Data Field. The frmNum field is set
	 * to the last frame transmitted plus one, and the ackNum field is set to the
	 * number of the next frame expected to be received. The reTx flag is clear.
	 * <li>The Data Field is exclusive-OR’ed with a pseudo-random sequence (see Data
	 * randomization).
	 * <li>The two-byte CRC of the Control Byte plus the Data Field is computed and
	 * appended after the Data Field.
	 * <li>The frame is byte stuffed to remove reserved byte values (see Reserved
	 * bytes and byte stuffing).
